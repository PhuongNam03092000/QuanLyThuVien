...
namespace Infrastructure.Persistence
{
    public class ChiTietPhieuMuonConfiguration : IEntityTypeConfiguration<ChiTietPhieuMuon>
    {
        public void Configure(EntityTypeBuilder<ChiTietPhieuMuon> builder)
        {
            ...
            builder.HasOne(pm => pm.PhieuMuon).WithMany(ctpm => ctpm.DSChiTietPhieuMuon).HasForeignKey(pm => pm.MaPM);
            builder.HasOne(s => s.Sach).WithMany(ctpm => ctpm.DSChiTietPhieuMuon).HasForeignKey(s => s.MaSach);
            builder.HasOne(s => s.Sach).WithMany(ctpm => ctpm.DSChiTietPhieuMuon).HasForeignKey(s => s.MaSach);
